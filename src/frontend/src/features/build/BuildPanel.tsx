...
export default function BuildPanel({ currentPou }: { currentPou?: string }) {
  ...
  const compileCurrentPou = async () => {
    ...
    const resPou = await fetch(`${baseUrl}/compile/pou?name=${encodeURIComponent(currentPou)}`).catch(() => null);
    if (!resPou || !resPou.ok) return;
    const data = await resPou.json();
    const t = data.text ?? ""; setPouText(t);
    const res = await fetch(`${baseUrl}/compile/st`, { method: "POST", headers: { "Content-Type": "text/plain" }, body: t }).catch(() => null);
    if (!res || !res.ok) return;
    const data2 = await res.json(); setPouDiags(data2.diagnostics ?? []);
  };

  const compileProject = async () => {
    const res = await fetch(`${baseUrl}/compile/project`).catch(() => null);
    if (!res || !res.ok) return;
    ...
    <div style={{ padding: 12, display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
      <div>
        ...
        <div style={{ marginTop: 8 }}>
          {diags.map((d, i) => (<div key={i} style={{ fontSize: 12 }}>{d.severity} L{d.Line}: {d.Message}</div>))}
        </div>
      </div>
      <div>
          ...
          <div style={{ marginTop: 12 }}>
            ...
            {projectResults.map((it, idx) => (
              <div key={idx} style={{ marginTop: 4 }}>
                <div>{it.name}</div>
                {it.diagnostics.map((d, i) => (<div key={i} style={{ fontSize: 12, paddingLeft: 8 }}>{d.severity} L{d.Line}: {d.Message}</div>))}
              </div>
            ))}
          </div>
        )}

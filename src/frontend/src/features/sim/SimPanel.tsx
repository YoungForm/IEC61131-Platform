...
export default function SimPanel() {
  ...
  const baseUrl = "http://localhost:5000"; // 如需调整端口，可在此修改

  const fetchVars = async () => {
    try {
      const res = await fetch(`${baseUrl}/sim/vars`);
  ...
  return (
    <div style={{ padding: 12 }}>
      <div style={{ marginBottom: 8 }}>
        <button onClick={async () => { await fetch(`${baseUrl}/sim/start?periodMs=${periodMs}`, { method: "POST" }); }}>启动仿真</button>
        <button onClick={async () => { await fetch(`${baseUrl}/sim/stop`, { method: "POST" }); }}>停止仿真</button>
      ...
      <div style={{ marginBottom: 8 }}>
        ...
        <button onClick={async () => { await fetch(`${baseUrl}/sim/set?name=${encodeURIComponent(name)}&value=${value}`, { method: "POST" }); fetchVars(); }}>设置变量</button>
      </div>
      <table>
